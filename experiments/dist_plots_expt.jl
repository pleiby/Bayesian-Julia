...
# ╔═╡ 6b8c85c9-4423-4858-92d5-318a9b253f4a
using SpecialFunctions

# ╔═╡ 0c13bcc9-dc96-4725-846d-366765e671f4
using Plots

# ╔═╡ bc532cd2-c75b-11ea-313f-8b5e771c9227
md"""# dist plots expt.jl
...
> ...
> In Bayesian inference, the beta distribution is the *conjugate prior probability distribution* for the Bernoulli, binomial, negative binomial and geometric distributions. The beta distribution is a suitable model for the random behavior of percentages and proportions. [Wikipedia, Beta Distribution](https://en.wikipedia.org/wiki/Beta_distribution)
...
$${\displaystyle f(x, \alpha, \beta) = {\frac {x^{\alpha -1}(1-x)^{\beta -1}}{\mathrm {B} (\alpha ,\beta )}}}$$
...
# ╔═╡ cfeb9eab-cb9e-4782-a72e-6b97bbba5dcc
md"""
...
# ╔═╡ de11bad6-82e7-4cca-845e-a551b655140b
md"""
Specifically, when estimating the parameter θ for a Bernoulli distribution, 

Posterior = Prior * Likelihood

p(θ | y) = p(θ) * p(y | θ) (to within the normalization or integration term p(y))
"""

# ╔═╡ dc561cc3-afd2-4135-a576-4fc530887df5
md"""
If the prior distribution for a binomial parameter θ is taken as a `beta` distribution (of θ over [0, 1])

prior p(θ) = beta(α, β) = ${\theta^{\alpha -1}(1-\theta)^{\beta -1} \over \mathrm {B} (\alpha ,\beta )}$

then the posterior distribution p(θ | y ) belongs to the same family of distributions as the prior distribution. I.e.

p(θ | y ) = beta(α, β) * binomial(y | n,θ)

is also of the `beta` form.

Note binomial(y | n,θ) is of the form

${\displaystyle p ( y | n, \theta ) = {n \choose s}q^{s}(1-q)^{n-s}}$
...
# ╔═╡ e97ed554-c60d-4447-b6dd-6c962ab4d51b
beta_params = ["α", "β"]

# ╔═╡ a5453888-3506-4182-a84d-aef30c66387e
SpecialFunctions.gamma(3.5)

# ╔═╡ 78ccd998-d6aa-4506-ab8a-f30a2e3eca0d
B(α, β) = gamma(α) * gamma(β)/ gamma(α+β)

# ╔═╡ ca0c055e-43de-4426-8212-36de151a1a49
beta(x, α, β) = (x^(α-1)*(1-x)^(β-1))/B(α, β)

# ╔═╡ e1a127ef-0fda-4c42-b5f6-8276adffca4c
beta(0.5, 1.0, 1.0)

# ╔═╡ 5cf25ace-8138-4cf7-b2b6-86ba8b5f9079
md"## Graphics for Dists"

# ╔═╡ 6e044754-4025-491e-a0e3-0cf3dc5ca48d
plotly()

# ╔═╡ 504cd1ce-b94b-4f71-a8f5-d8058b539a0f
plot(0.5:0.25:5, gamma.(0.5:0.25:5), legend=false, title="Γ Function")

# ╔═╡ 8c3c0494-2416-411a-9e25-59da66230a8b
xrange = 0.01:0.01:1.0

# ╔═╡ 5102767c-958e-4d9f-a619-281651dfc601
md"### Slider Aside
We can show a slider followed by its current value (but not on the same line?)

The first argument is a `Vector` or range. You can set the _default value_ using a keyword argument:"

# ╔═╡ aa2b03b2-cc4a-44ed-8cbe-a700c8cee1ce
md"$(@bind y Slider(20:0.1:30, default=25);) $y"

# ╔═╡ dd851756-ba27-47a2-87d8-9a0398871982
md"""
**In Bayesian estimation,** the posterior distribution looks a lot like the likelihood function p(y | θ), when using non-informative priors. Alternatively, when using highly-informative (tight) priors, the posterior distribution will look similar to the prior distribution, if the sample size is small.

More informative priors have a stronger influence on the posterior, for a given sample size. 

Likelihood functions are more influenctial on posteriors for larger sample sizes.
"""
...
# ╔═╡ 77ee6c22-f7b9-4381-b352-a8fe8c98bd8a
@bind x Slider(20:0.1:30, default=25)
...
# ╔═╡ 3912a922-8af4-41e4-8b92-a7ab9b325fa6
"""
	beta_dist_params_input(param_names::Vector)

generates combined sliders, one for each of names in `param_names`
"""
function beta_dist_params_input(param_names::Vector)
...
# ╔═╡ 58e716cc-d1c7-4370-9fb1-87eb8124d5dd
@bind beta_param_values beta_dist_params_input(beta_params)

# ╔═╡ ea491a77-d9ae-4deb-86af-f7637fdb57bb
begin
	plot(xrange, beta.(xrange, beta_param_values.α, beta_param_values.β),
		label = "beta$beta_param_values")
	plot!(legend=false, title="beta(x, α, β) Function")
	plot!(xrange, beta.(xrange, 1, beta_param_values.β),
		label = "beta(1, $(beta_param_values.β) )")
	plot!(xrange, beta.(xrange, beta_param_values.α, 1),
		label = "beta( $(beta_param_values.α), 1)")
end

# ╔═╡ 3473276f-ebe4-467c-9951-df62562e1c3f
beta_param_values
...
# ╔═╡ 7ccd94c6-0790-4554-b576-d31d9a48bc5f
